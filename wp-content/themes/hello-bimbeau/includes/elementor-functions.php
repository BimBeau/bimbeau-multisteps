...
add_action( 'elementor/init', 'hello_bimbeau_settings_init' );

function hello_bimbeau_settings_init() {
	if ( hello_header_footer_experiment_active() ) {
		...
		add_action( 'elementor/kit/register_tabs', function( \Elementor\Core\Kits\Documents\Kit $kit ) {
			$kit->register_tab( 'hello-settings-header', HelloBimBeau\Includes\Settings\Settings_Header::class );
			$kit->register_tab( 'hello-settings-footer', HelloBimBeau\Includes\Settings\Settings_Footer::class );
		}, 1, 40 );
	}
}
 ...
 */
function hello_bimbeau_get_setting( $setting_id ) {
	global $hello_bimbeau_settings;

	$return = '';

	if ( ! isset( $hello_bimbeau_settings['kit_settings'] ) ) {
		$kit = \Elementor\Plugin::$instance->kits_manager->get_active_kit();
		$hello_bimbeau_settings['kit_settings'] = $kit->get_settings();
	}

	if ( isset( $hello_bimbeau_settings['kit_settings'][ $setting_id ] ) ) {
		$return = $hello_bimbeau_settings['kit_settings'][ $setting_id ];
	}

	return apply_filters( 'hello_bimbeau_' . $setting_id, $return );
}
...
function hello_show_or_hide( $setting_id ) {
	return ( 'yes' === hello_bimbeau_get_setting( $setting_id ) ? 'show' : 'hide' );
}
...
function hello_get_header_layout_class() {
	...
	$header_layout = hello_bimbeau_get_setting( 'hello_header_layout' );
	...
	$header_width = hello_bimbeau_get_setting( 'hello_header_width' );
	if ( 'full-width' === $header_width ) {
		$layout_classes[] = 'header-full-width';
	}

	$header_menu_dropdown = hello_bimbeau_get_setting( 'hello_header_menu_dropdown' );
	...
	$hello_header_menu_layout = hello_bimbeau_get_setting( 'hello_header_menu_layout' );
...
function hello_get_footer_layout_class() {
	$footer_layout = hello_bimbeau_get_setting( 'hello_footer_layout' );
	...
	$footer_width = hello_bimbeau_get_setting( 'hello_footer_width' );

	if ( 'full-width' === $footer_width ) {
		$layout_classes[] = 'footer-full-width';
	}

	if ( hello_bimbeau_get_setting( 'hello_footer_copyright_display' ) && '' !== hello_bimbeau_get_setting( 'hello_footer_copyright_text' ) ) {
...
function hello_get_header_display() {
	...
	return (
		$is_editor
		|| hello_bimbeau_get_setting( 'hello_header_logo_display' )
		|| hello_bimbeau_get_setting( 'hello_header_tagline_display' )
		|| hello_bimbeau_get_setting( 'hello_header_menu_display' )
	);
}
...
function hello_get_footer_display() {
	...
	return (
		$is_editor
		|| hello_bimbeau_get_setting( 'hello_footer_logo_display' )
		|| hello_bimbeau_get_setting( 'hello_footer_tagline_display' )
		|| hello_bimbeau_get_setting( 'hello_footer_menu_display' )
		|| hello_bimbeau_get_setting( 'hello_footer_copyright_display' )
	);
}

/**
 * Add Hello BimBeau theme Header & Footer to Experiments.
 */
add_action( 'elementor/experiments/default-features-registered', function( \Elementor\Core\Experiments\Manager $experiments_manager ) {
	$experiments_manager->add_feature( [
		...
		'title' => esc_html__( 'Hello Theme Header & Footer', 'hello-bimbeau' ),
		'description' => sprintf( __( 'Use this experiment to design header and footer using Elementor Site Settings. <a href="%s" target="_blank">Learn More</a>', 'hello-bimbeau' ), 'https://go.elementor.com/wp-dash-header-footer' ),
