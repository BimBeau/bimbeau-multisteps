...
<header id="site-header" class="site-header dynamic-header <?php echo esc_attr( hello_get_header_layout_class() ); ?>" role="banner">
	<div class="header-inner">
		<div class="site-branding show-<?php echo esc_attr( hello_bimbeau_get_setting( 'hello_header_logo_type' ) ); ?>">
			<?php if ( has_custom_logo() && ( 'title' !== hello_bimbeau_get_setting( 'hello_header_logo_type' ) || $is_editor ) ) : ?>
				<div class="site-logo <?php echo esc_attr( hello_show_or_hide( 'hello_header_logo_display' ) ); ?>">
					<?php the_custom_logo(); ?>
				</div>
			<?php endif;

			if ( $site_name && ( 'logo' !== hello_bimbeau_get_setting( 'hello_header_logo_type' ) || $is_editor ) ) : ?>
			...
			if ( $tagline && ( hello_bimbeau_get_setting( 'hello_header_tagline_display' ) || $is_editor ) ) : ?>
		...
		<?php if ( $header_nav_menu ) : ?>
			...
			<div class="site-navigation-toggle-holder <?php echo esc_attr( hello_show_or_hide( 'hello_header_menu_display' ) ); ?>">
				<div class="site-navigation-toggle" role="button" tabindex="0">
					...
					<span class="screen-reader-text"><?php echo esc_html__( 'Menu', 'hello-bimbeau' ); ?></span>
				</div>
			</div>
