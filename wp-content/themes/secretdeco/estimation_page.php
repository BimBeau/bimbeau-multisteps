...
// Définit le mode Stripe : 'TEST' ou 'PROD'
$modeStripe = 'PROD'; // Changez cette valeur en 'PROD' pour utiliser le mode production

// Configuration des options Stripe en fonction du mode choisi
if ($modeStripe === 'PROD') {
    $GLOBALS['stripeOptions'] = [
        'payment-link' => 'https://buy.stripe.com/14k5mzfDf86f7U4cO6',
        'secret-key' => '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'
    ];
} else { // Mode TEST par défaut
    $GLOBALS['stripeOptions'] = [
        'payment-link' => 'https://buy.stripe.com/test_bIY2bbckteyjgbm4gg',
        'secret-key' => 'sk_test_51JUCdyHKX5FyumXs1WF9dsIgDPgJu2a05VtBgspxxA86CDwrkGy3cPadlSXx9LyZhP5iDitOcQ8m62dvEgsWESoT007cVCjJiA'
    ];
}

// Options générales
$GLOBALS['generalOptions'] = [
    'admin-email' => 'hello@secretdeco.fr',
];
...
// Options des besoins spécifiques
    ...
    switch ($etape) {
        case '1':
            ...
            foreach ($GLOBALS['profilOptions'] as $value => $label) {
                ...
                $radio_column_html = str_replace('<div class="elementor-icon-box-wrapper">', '<div class="elementor-icon-box-wrapper"><div class="elementor-icon-box-icon"><span class="elementor-icon elementor-animation-">' . file_get_contents(get_stylesheet_directory() . '/assets/img/estimation/' . $value . '.svg') . '</span></div>', $radio_column_html);
...
/**
 * Vérifie si un identifiant de session est valide en interrogeant l'API Stripe.
 */
function isSessionIdValid($session_id) {
    // Crée une requête cURL pour interagir avec l'API Stripe
    ...
    if (isset($session['error'])) {
        return false; // Si $session contient une erreur, retournez false
    } elseif ($session && $session['payment_status'] == 'paid') {
        return true; // La session est payée et valide
    } else {
        return false; // La session de paiement n'est pas valide
    }
}
...
function isPreviousStepCompleted($current_step) {
    // Mise à jour du mapping des étapes aux clés de session pour inclure la 11ème étape
    $step_keys = [
        '1' => 'profil',
        '2' => 'projet',
        '3' => 'accompagnement',
    ...
    // Si l'étape actuelle est la première, pas besoin de vérifier
    if ($current_step === '1') {
        return true;
    }

    // Trouver l'index de l'étape précédente
...
// // Affichage des données de la variable de session
// display('SESSION ESTIMATION');
// if (isset($_SESSION['estimation'])) {
//     // Utilisez la fonction display pour afficher les données
//     display($_SESSION['estimation']);
// } else {
//     echo 'Aucune donnée disponible SESSION ESTIMATION';
// }

// // Affichage des données de la variable GET
// display('POST');
// if (isset($_POST)) {
//     // Utilisez la fonction display pour afficher les données
//     display($_POST);
// } else {
//     echo 'Aucune donnée disponible POST';
// }

// // Affiche le détail de la demande à partir des données renseignées
// if (isset($_SESSION['estimation']) && !empty($_SESSION['estimation'])) {
//     // Récupère les données de la session d'estimation
//     $estimation = $_SESSION['estimation'];

//     // Commence l'affichage de la liste
//     echo '<ul>';

//     // Affiche le profil
//     echo '<li>Profil : ' . $GLOBALS['profilOptions'][$estimation['profil']] . '</li>';

//     // Affiche le projet
//     echo '<li>Projet : ' . $GLOBALS['projetOptions'][$estimation['projet']] . '</li>';

//     // Affiche l'accompagnement
//     echo '<li>Accompagnement : ' . $GLOBALS['accompagnementOptions'][$estimation['accompagnement']]['label'] . '</li>';

//     // Affiche la superficie
//     echo '<li>Superficie : ' . $GLOBALS['superficieOptions'][$estimation['superficie']] . '</li>';

//     // Affiche les besoins
//     echo '<li>Besoins : ';
//     echo '<ul>';
//     foreach ($estimation['besoins'] as $besoin) {
//         echo '<li>' . $GLOBALS['besoinsOptions'][$estimation['accompagnement']][$besoin] . '</li>';
//     }
//     echo '</ul>';
//     echo '</li>';

//     // Affiche les infos complémentaires
//     echo '<li>Informations complémentaires : ' . $estimation['infos-complementaires'] . '</li>';

//     // Affiche le démarrage
//     if ($estimation['demarrage']['delai'] === 'date-precise') {
//         echo '<li>Démarrage : À une date précise, le ' . $estimation['demarrage']['date-demarrage'] . '</li>';
//     } else {
//         echo '<li>Démarrage : ' . $GLOBALS['demarrageOptions'][$estimation['demarrage']['delai']] . '</li>';
//     }

//     // Affiche le budget
//     echo '<li>Budget : ' . $GLOBALS['budgetOptions'][$estimation['budget']] . '</li>';

//     // Affiche les coordonnées
//     echo '<li>Coordonnées : ';
//     echo '<ul>';
//     echo '<li>Prénom : ' . $estimation['coordonnees']['prenom'] . '</li>';
//     echo '<li>Nom : ' . $estimation['coordonnees']['nom'] . '</li>';
//     echo '<li>Email : ' . $estimation['coordonnees']['email'] . '</li>';
//     echo '<li>Téléphone : ' . $estimation['coordonnees']['telephone'] . '</li>';
//     echo '<li>Adresse : ' . $estimation['coordonnees']['adresse'] . '</li>';
//     echo '</ul>';
//     echo '</li>';

//     // Termine l'affichage de la liste
//     echo '</ul>';
// } else {
//     // Si la session d'estimation est vide ou n'existe pas, affiche un message d'erreur
//     echo 'Aucune estimation trouvée.';
// }
...
function custom_log($message) { 
    ...
    // Construction du message complet à enregistrer
...
\Elementor\Plugin::$instance->frontend->add_body_class('estimation');
...
<head>
    ...
    <style>
        .grecaptcha-badge,
        body[data-elementor-device-mode="mobile"] #estimation-standard-submit a .elementor-button-icon,
        body[data-elementor-device-mode="mobile"] #estimation-express-submit a .elementor-button-icon,
        body[data-elementor-device-mode="mobile"] .badge-estimation .elementor-icon-box-content,
        body[data-elementor-device-mode="mobile"] #estimation-prev .elementor-button-text {
            display: none;
        }

        body[data-elementor-device-mode="mobile"] #estimation-prev .elementor-button-link {
            background-color: var(--e-global-color-cf746cf);
        }

        .efs-subtitle {
            font-family: var(--e-global-typography-e459aae-font-family), Sans-serif;
            font-size: var(--e-global-typography-e459aae-font-size);
            font-weight: var(--e-global-typography-e459aae-font-weight);
            text-transform: var(--e-global-typography-e459aae-text-transform);
            line-height: var(--e-global-typography-e459aae-line-height);
            letter-spacing: var(--e-global-typography-e459aae-letter-spacing);
            word-spacing: var(--e-global-typography-e459aae-word-spacing);
        }

        .estimation_form_step {
            display: flex;
            flex-wrap: wrap;
            margin: 0;
        }

        .estimation_form_step .elementor-icon-box-wrapper .elementor-icon-box-icon {
            display: block !important;
        }

        .estimation_form_step.no-icon .elementor-icon-box-icon,
        .estimation_form_step .elementor-icon-box-wrapper .elementor-icon-box-icon:not(:first-child) {
            display: none !important;
        }

        .estimation_form_step.no-icon div[data-elementor-type="section"]>div.elementor-element {
            align-items: start;
        }
        ...
        .estimation_form_step div[data-elementor-type="section"] {
            display: flex;
        }

        #mes-coordonnees-form.estimation_form_step {
            margin-bottom: var(--gap_M);
        }

        .estimation_form_step.efs_columns div[data-elementor-type="section"] {
            flex-basis: calc((100% - (var(--gap_M)*2)) / 3);
        }

        #mes-coordonnees-form.estimation_form_step>div.efs_col_50,
        body[data-elementor-device-mode="laptop"] #besoins-form.estimation_form_step div[data-elementor-type="section"],
        body[data-elementor-device-mode="tablet"] #besoins-form.estimation_form_step div[data-elementor-type="section"],
        body[data-elementor-device-mode="mobile"] .estimation_form_step.efs_columns div[data-elementor-type="section"] {
            flex-basis: calc((100% - (var(--gap_M)*2)) / 2);
        }

        body[data-elementor-device-mode="mobile"] #mes-coordonnees-form.estimation_form_step>div.efs_col_50,
        #mes-coordonnees-form.estimation_form_step>div.efs_col_100,
        body[data-elementor-device-mode="mobile"] #besoins-form.estimation_form_step div[data-elementor-type="section"] {
            flex-basis: 100%;
        }
        ...
        body[data-elementor-device-mode="mobile"] .efs_gap_M,
        body[data-elementor-device-mode="mobile"] .efs_gap_S {
            gap: 10px;
        }
        ...
        .estimation_form_step .efs_field_description {
            color: var(--e-global-color-84d7aad);
            font-family: var(--e-global-typography-42551c2-font-family), Sans-serif;
            font-size: var(--e-global-typography-42551c2-font-size);
            font-weight: var(--e-global-typography-42551c2-font-weight);
            text-transform: var(--e-global-typography-42551c2-text-transform);
            font-style: var(--e-global-typography-42551c2-font-style);
        ...
        /* Masquer les inputs spécifiques */
        .estimation_form_step input[type="radio"],
        .estimation_form_step input[type="checkbox"] {
            display: none;
        }

        /* Style pour l'élément parent lorsque le champ enfant est actif */
        .estimation_form_step div[data-elementor-type="section"].active>div[data-element_type="container"] {
            border-color: var(--e-global-color-text);
            background: var(--e-global-color-cf746cf);
        }

        .estimation_form_step.efs_minheight_S div[data-elementor-type="section"]>div[data-element_type="container"] {
            min-height: var(--minheight_S);
        }
        ...
        .estimation_form_step .custom-input {
            display: flex;
            gap: 15px;
            align-items: center;
            padding: 0 10px;
        ...
        .estimation_form_step .custom-input input {
            display: none;
        }

        .estimation_form_step .custom-input.custom-input-checkbox::before,
        .estimation_form_step .custom-input.custom-input-radio::before {
            content: '';
            display: block;
            width: 22px;
            min-width: 22px;
            height: 22px;
            min-height: 22px;
            background-color: var(--e-global-color-a621b80);
            border: 3px solid var(--e-global-color-a621b80);
            box-shadow: 0 0 0 1px var(--e-global-color-ca7eba9);
        }
        ...
        .estimation_form_step div[data-elementor-type="section"].active .custom-input::before {
            background-image: url('<?php echo get_stylesheet_directory_uri(); ?>/assets/img/check_form.svg');
        ...
        .estimation_form_step label {
            font-family: var(--e-global-typography-d35e44d-font-family), Sans-serif;
            font-size: var(--e-global-typography-d35e44d-font-size);
            font-weight: var(--e-global-typography-d35e44d-font-weight);
            text-transform: var(--e-global-typography-d35e44d-text-transform);
            line-height: var(--e-global-typography-d35e44d-line-height);
            letter-spacing: var(--e-global-typography-d35e44d-letter-spacing);
            word-spacing: var(--e-global-typography-d35e44d-word-spacing);
            color: var(--e-global-color-text);
            margin: 10px 0;
        }

        .estimation_form_step.efs_flex_column {
            flex-direction: column;
        }

        .estimation_form_step textarea,
        .estimation_form_step input {
            border-radius: 15px;
            border-color: var(--e-global-color-ca7eba9);
            border-width: 1.5px;
            outline: none;
        }

        .estimation_form_step textarea {
            padding: min(calc(2vw + 10px), 70px);
        }

        .estimation_form_step textarea:focus,
        .estimation_form_step input:focus {
            border-color: var(--e-global-color-text);
        }

        .estimation_form_step textarea,
        .estimation_form_step textarea::placeholder,
        .estimation_form_step input,
        .estimation_form_step input::placeholder {
            color: var(--e-global-color-text);
        ...
        .efs-animate-out {
            animation: animOutEstimation 0.6s forwards;
        }
    </style>
    <!-- Gestion des interactions avec les champs de formulaire dans Elementor -->
    <script>
        (function($) {
            "use strict";
            ...
            $(window).on('elementor/frontend/init', function() {
                ...
                // Fonction pour vérifier et activer la classe 'active' sur l'élément parent si nécessaire
                ...
                // Appliquer cette vérification au chargement
                ...
                $(document).on('click', '.estimation_form_step div[data-elementor-type="section"]', function() {
                    ...
                    // Modifier l'état ou la valeur du champ
                    ...
                    // Optionnel : Pour les boutons radio, retirer la classe active des éléments frères
                    if ($field.is(':radio')) {
                        $this.siblings().removeClass('active');
                    }
                });

                // Animation de sortie pour chaque étape avant redirection
                function animateAndRedirect(url, delay, isFormSubmission, isFinalSubmission) {
                    $('#ef_step').addClass('efs-animate-out');
                    setTimeout(function() {
                        if (isFormSubmission) {
                            if (isFinalSubmission) {
                                // Si c'est la soumission finale, attendre reCAPTCHA
                                grecaptcha.ready(function() {
                                    grecaptcha.execute("6LdmaecoAAAAADI-XWX738fvkmXIN3Oq0lXqZutN", {
                                        action: "submit"
                                    }).then(function(token) {
                                        $('#recaptchaResponse').val(token); // Assigner le token à l'input caché
                                        $('form').submit(); // Soumettre le formulaire après avoir récupéré le token
                                    });
                                });
                            } else {
                                // Pour les autres soumissions de formulaire
                ...
                $('a').on('click', function(e) {
                    ...
                    if (url && url !== '#' && !$(this).hasClass('no-animation')) {
                        ...
                        animateAndRedirect(url, 600, false, false); // Redirection après l'animation
                    }
                });

                // Gérer le clic sur le lien "Suivant"
                $('#estimation-next a').on('click', function(e) {
                    e.preventDefault(); // Empêcher le comportement par défaut du lien
                    animateAndRedirect(null, 600, true, false); // Pas d'URL, juste une soumission de formulaire
                });

                // Gérer le clic sur le lien "Soumettre" à l'étape finale
                $('#estimation-standard-submit a, #estimation-express-submit a').on('click', function(e) {
                    e.preventDefault(); // Empêcher le comportement par défaut du lien
                    animateAndRedirect(null, 600, true, true); // Soumission finale avec reCAPTCHA
                });
